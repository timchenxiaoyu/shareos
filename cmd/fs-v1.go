...
func migrateFSObject(fsPath, fsUUID string) (err error) {
	...
	println("Please do not stop the server.")
	...
	go func() {
		for {
			...
			println("Please wait server is being upgraded..")
		}
	}()
...
func migrateFSFormatV1ToV2(fsPath, fsUUID string) (err error) {
	...
	fmt.Printf("Migrating bucket metadata format from \"%s\" to newer format \"%s\"... completed successfully.", fsFormatV1, fsFormatV2)
...
func (fs fsObjects) StorageInfo() StorageInfo {
	...
	println(err, "Unable to get disk info %#v", fs.fsPath)
...
func (fs fsObjects) CopyObject(srcBucket, srcObject, dstBucket, dstObject string, metadata map[string]string) (ObjectInfo, error) {
	...
	go func() {
		...
		if gerr := fs.GetObject(srcBucket, srcObject, startOffset, length, pipeWriter); gerr != nil {
			println(gerr, "Unable to read %s/%s.", srcBucket, srcObject)
...
func (fs fsObjects) PutObject(bucket string, object string, size int64, data io.Reader, metadata map[string]string, sha256sum string) (objInfo ObjectInfo, retErr error) {
	...
	if err != nil {
		...
		println(err, "Failed to create object %s/%s", bucket, object)
		return ObjectInfo{}, toObjectErr(err, bucket, object)
	}
